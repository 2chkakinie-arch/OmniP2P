/*!
 * ============================================================================
 *  OmniP2P.js v2  —  The Everything P2P Communication & Persistence Engine
 * ============================================================================
 *  Version : 2.0.0
 *  License : MIT
 ...
 *
 *  ---------------------------------------------------------------------------
 *  v2 での変更点 (WHAT'S NEW) — Manus レビュー全指摘への回答
 *  ---------------------------------------------------------------------------
 *   コアは「軽量な通信・保存エンジン」に絞り、重い外部依存 (Gun/Matrix/Yjs/
 *   Waku 等) は "必要な時だけ CDN から遅延ロードする外部アダプタ" に分離。
 *
 *   外部アダプタ (jsDelivr から自動ロード, 明示 use() で有効化):
 *     OmniP2P.loadAdapter('gundb')   // GunDB.js  リアルタイムグラフ同期
 *     OmniP2P.loadAdapter('matrix')  // Matrix.js Megolm E2EE 永続ルーム
 *     OmniP2P.loadAdapter('yjs')     // Yjs.js    本物の Y.Doc プロバイダ
 *     OmniP2P.loadAdapter('waku')    // Waku.js   並列匿名搬送 + Store 永続
 *   → いずれも https://cdn.jsdelivr.net/gh/2chkakinie-arch/OmniP2P@main/<Name>.js
 *
 *   安定性・速度の強化 (レビュー指摘 #1〜#12 をすべて修正):
 *    #1  ハートビートは EOSE 応答を実確認してから "生存" とみなす (half-open 検知)
 *    #2  watchdog はアプリイベント受信のみでリセット (ハートビート応答では ×)
 *    #3  リレー再接続をスタガー + 「最低1本維持」フェイルセーフで瞬断ゼロ化
 *    #4  Room.send のフォールバック判定を pc.open 実数ベースに統一
 *    #5  SDP/ICE 等の重要イベントは minAck 複数 + ack 不足時の自動再送
 *    #6  保留 ICE 候補にリングバッファ上限 (重複配送の氾濫を抑制)
 *    #7  polite 側も restart タイムアウト後に re-offer 要求へフォールバック
 *    #8  RelayConnection と PeerConnection の再接続を状態機械で協調 (glare 低減)
 *    #9  DataChannel bufferedAmount バックプレッシャー流量制御
 *    #10 再接続時に since を「最後に受信した時刻」に更新し取りこぼし防止
 *    #11 RTC が connected なら presence timeout だけでは切断しない (RTC 優先)
 ...
 *    ハイブリッド、保存はコンテンツアドレス指定 + マルチプロバイダ冗長化。
 * ============================================================================
 */
